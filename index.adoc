...
* **大尚国际商城（2015.12——至今）**
    ...
    ** **项目职责**
        *** **2019至2020年**
            **** 1、协助同事解决跨域登录无法请求数据问题。通过使用Memcached替代Session存储用户登录信息方案，解决特殊情况下，需要从商城前台往后台跨域登录成功却无法再请求活动数据的问题。原因是跨域的每个请求会生成一个Session（基于SessionID判定）导致登录信息丢失。在开发和测试环境没发现问题，原因是开发测试与生产环境不一致，即开发测试环境习惯使用单个tomcat运行多个项目，并没有产生跨域访问。
